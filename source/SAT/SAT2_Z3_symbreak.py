...
# Lexicographic less or equal comparison between two Boolean vectors
def lex_lessequal(a, b):
    assert len(a) == len(b)
    n = len(a)
    strict_cases = [
        And(*(a[j] == b[j] for j in range(i)), Not(a[i]), b[i])
        for i in range(n)]

    equal_case = And(*(a[j] == b[j] for j in range(n)))  # Case in which a is equal to b
    return Or(equal_case, *strict_cases)
...
# Generate a SAT model to solve the tournament scheduling problem, using the Z3 solver and some symmetry breaking constraints to reduce the
...
def tournament_SAT_scheduler(teams):
  ...
  for p in range(number_of_periods):
    s.add(is_home[p][0][p]==True)
    s.add(is_away[p+number_of_periods][0][p]==True)

  # Symmetry breaking constraint: lexicographical ordering between columns (weeks) starting from week 2, since the first week is already fixed
  for w in range(number_of_weeks - 1):
    week_i = [is_home[x][w][p] for x in range(teams-1,-1,-1)for p in range(number_of_periods)]
    week_i_plus_1 = [is_home[x][w+1][p] for x in range(teams-1,-1,-1)for p in range(number_of_periods)]
    s.add(lex_lessequal(week_i, week_i_plus_1))

  # Symmetry breaking constraint: lexicographical ordering between teams starting from week 2, since the first week is already fixed
  for w in range(1,number_of_weeks):
    for p in range(number_of_periods):
      for x in range(teams):
        for y in range(teams):
          if x<y:
            s.add(Or(Not(is_home[x][w][p]), Not(is_away[y][w][p])))  

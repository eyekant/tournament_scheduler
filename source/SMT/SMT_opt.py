...
# Generate a SMT model to solve the tournament scheduling problem, using the Z3 solver
def tournament_SMT_scheduler(teams):
  ...
  # Soft constraint for a fair assignment: try to avoid consecutive home matches for the same team
  for x in range(1,teams+1):
    for w in range(number_of_weeks-1):
        s.add_soft(Not(And(Or([home_team[w][p]==x for p in range(number_of_periods)]),
                           Or([home_team[w+1][p]==x for p in range(number_of_periods)]))),weight = 10)

  # Soft constraint for a fair assignment: try to avoid consecutive away matches for the same team
  for x in range(1,teams+1):
    for w in range(number_of_weeks-1):
      s.add_soft(Not(And(Or([away_team[w][p]==x for p in range(number_of_periods)]),
                           Or([away_team[w+1][p]==x for p in range(number_of_periods)]))),weight = 10)

  # Soft constraint for a fair assignment: assuming the last period represents the weekend, try to ensure that all teams have a balanced number of weekend games
  total_weekend_matches = (teams-1)//number_of_periods
  for x in range(1,teams+1):
    weekend_matches = [Or(home_team[w][-1]==x,away_team[w][-1]==x)
                       for w in range(number_of_weeks)] # Consider the last period (= index -1) as weekend
    total = Sum(If(g,1,0)for g in weekend_matches)
    s.add_soft(total >= total_weekend_matches, weight = 2)
    s.add_soft(total <= total_weekend_matches+1, weight = 2)

  # For each team, compute the number of matches supposed to be played at home during the tournament
  home_count = [
      Sum([
          If(home_team[w][p] == t, 1, 0)
          for w in range(number_of_weeks)
          for p in range(number_of_periods)
      ])
      for t in range(1, teams + 1)]

  # Since each team plays teams - 1 matches, which is odd, the maximum and minimum number of home matches should be included in this range
  max_home = Int("max_home")
  min_home = Int("min_home")
  for hc in home_count:
      s.add(hc <= max_home)
      s.add(hc >= min_home)

  # Balance the number of home and away games
  s.minimize(max_home - min_home)
  ...
  if result == unsat:
    ...
    obj_val = m.evaluate(max_home - min_home).as_long()
    ...
    return {"time": 300, "optimal": False, "obj":obj_val, "sol":matrix}
    ...
    for team1, team2, w, p in schedule:
        matrix[p][w] = [team1, team2]
    return {"time": elapsed, "optimal": True, "obj":obj_val, "sol":matrix}
